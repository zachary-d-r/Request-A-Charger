import Student
import eel

student = Student.student(backend=False)  # Create student object to load file

eel.init('web')  # Initialize the app

# Function to print a return value from js
def printReturn(n):
        print(n)

# Function to get student id when the submit button is clicked
@ eel.expose
def getStudentID():
    eel.getStudentIDJS()()  # The () after the function gets the return values
    student.studentID = eel.getStudentIDJS()()

    student.loadIntoFile(student.studentID)  # Loads the student id onto the file


eel.start('index.html')  # Start the app
